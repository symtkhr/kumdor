...
$.ajax({
    url: "wordlist.10000.ja.txt",
    type: 'get',
    success: (data) => {
        Spellcast.words = data.split("\n").map(row => {
            let cell = row.split("\t");
            if (cell.length != 3) return;
            let w = cell[2].trim().split("").map(c => {
                let u = c.charCodeAt(0)  + (c.match(/[ぁ-ん]/) ? 0x60 : 0);
                if (u < "ァ".charCodeAt(0) || "ー".charCodeAt(0) < u) return c;
                const p = "ｧｱｨｲｩｳｪｴｫｵｶ ｷ ｸ ｹ ｺ ｻ ｼ ｽ ｾ ｿ ﾀ ﾁ ｯﾂ ﾃ ﾄ ﾅﾆﾇﾈﾉﾊ ﾟﾋ ﾟﾌ ﾟﾍ ﾟﾎ ﾟﾏﾐﾑﾒﾓｬﾔｭﾕｮﾖﾗﾘﾙﾚﾛ_ﾜ__ｦﾝ_______･-";
                let idx = u - "ァ".charCodeAt(0);
                let co = p.slice(idx, idx + 1);
                if (co == " ") return p[idx - 1] + "ﾞ";
                if (co == "ﾟ") return p[idx - 2] + "ﾟ";
                if (co == "_") return c;
                return co || c;
            }).join("");
            return w;
...
let jumon = (level) => {
    ...
    const make_wordcard = () => {
        ...
        cs += Keyboard.shift(cs) + " ";
        cs = Keyboard.tokana(cs);
        let words = (2 < level) ? Spellcast.words : l2.map(w => Keyboard.tokana(w));
